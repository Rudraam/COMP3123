...
`express.Router()` is used to create modular, mountable route handlers. It allows you to organize routes into separate files and modules, making the code more maintainable and structured. Benefits include:
-Modularity: Routes can be organized by functionality (users, products, etc.)
-Reusability**: Router instances can be mounted at different paths
-Middleware: Each router can have its own middleware stack
-Clean Architecture: Separates concerns and keeps the main app file clean
...
Error handling in Express can be implemented using:
1.Try-catch block in async routes
2.Error-handling middleware with 4 parameters (err, req, res, next)
3.Status codes for different error types
...
The `app.listen(process.env.port || 8081)` line implements dynamic port binding:
-process.env.port: Reads the PORT environment variable set by the hosting platform
-|| 8081: Uses 8081 as fallback if no environment variable is set
-Production Benefits:
  - Cloud platforms (Heroku, AWS, etc.) assign random available ports
  - Prevents port conflicts in containerized environments
  - Allows easy configuration without code changes
  - Supports different ports for development, staging, and production
